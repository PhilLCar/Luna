...
Les chaînes de caractères sont représentées par une case mémoire (8 bytes) indiquant la taille de la séquence de byte qui suit (moins le caractère nul). La fin du string contient au moins un caractère nul, mais potentiellement plus pour être aligné avec la mémoire. Exemple:
\begin{center}//Bonjour tous
  \begin{table}
  \begin{tabular}{|c|}
    \hline
    ...
    \hline
  \end{tabular}
  \caption{Exemple de la représentation en mémoire du string {\lstset{style=lua}\lstinline$"Bonjour tous"$}}
  \end{table}
\end{center}
\paragraph{Tables - \texttt{011}}
\paragraph{Objets - \texttt{100}}
\setcounter{subparagraph}{-1}
\subparagraph{Fermetures - \texttt{0100}}
\subparagraph{Fichiers - \texttt{1100}}
\paragraph{Pile - \texttt{101}}
